...
It seems reasonable to assume \(\sigma_{u}^{2}\) = \(\sigma_{r}^{2}\). Consequently, we apply a **pooled standard deviation** for our estimates<sup></sup>^[We can assume equal variances when the ratio of max/min is less than 3 or less than 4 for small samples [@taylor_math5741m:_2017, p.69].].
...
We compare this to the critical point of t-distribution with \(\nu\) = 198,643 degrees of freedom<sup></sup>^[With this number of degrees of freedom we could have also apply z-statistic and the result would have been nearly the same.], which is \(t_{198643}\)(0.005)=2.575854. Since |0.001374182| < 2.575854, we do not reject the null hypothesis and conclude that the mean of vehicles involved in both types of areas is equal (\(\mu_{u}\) = \(\mu_{r}\)).
